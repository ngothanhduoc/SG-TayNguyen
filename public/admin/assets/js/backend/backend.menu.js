...
var BACKEND = {
    API_URL_LIST: '/backend/list/function',
    AJAX_URL_DELETE: '/backend/ajax/delete/admin_menu/index/function/id_function',
    AJAX_URL_UPDATE: '/backend/ajax/updatestatus/admin_menu/index/function/id_function',
        ...
        var dataAdapter = new $.jqx.dataAdapter(source, {
            beforeLoadComplete: function (records) {
                ...
                for (var i = j; i < records.length; i++) {
                    records[i].idCoppy = records[i].id_function + ',' + records[i].sellmore;
                    records[i].groName = arrGroup[records[i].parent_id];
					
                    records[i].idsort = records[i].id_function + ',' + records[i].sort;
                    records[i].idStatus = records[i].id_function + ',' + records[i].is_leaf;
    ...
    init: function() {
	    ...
	    columns: [
                ...
                {text: 'NAME', datafield: 'name_display', width: 150},
		{text: 'ALIAS', datafield: 'alias', width: 150},
		{text: 'URL', datafield: 'url', width: 200},
		{text: 'GROUP ID', datafield: 'parent_id'},
		{text: 'GROUP NAME', datafield: 'groName',filterable: false, sortable: false},
		{text: 'STATUS', datafield: 'idStatus', cellsrenderer: BACKEND.statuscolumnrender, width: 60,filterable: false, sortable: false},
                {text: 'CÔNG CỤ', datafield: 'id_function', cellsalign: 'center', align: 'center', cellsrenderer: BACKEND.toolscolumnrender, width: 80, sortable: false, filterable: false},
            ],
            virtualmode: true,
    ...
    gridEdit: function(id) {
        //loadPopup(id, false);
        window.location.href = '/backend/menu/add?id=' + id;
    },
    setBc: function(id){
        $.ajax({
	...
	desable: function(id){
        $.ajax({
                url: BACKEND.AJAX_URL_UPDATE + '?id=' + id + '&st=0&field=is_leaf',
                type: 'GET',
                dataType: 'JSON',
                data: {}
            }).done(function(response) {
	...
	enable: function(id){
        $.ajax({
                url: BACKEND.AJAX_URL_UPDATE + '?id=' + id + '&st=1&field=is_leaf',
                type: 'GET',
                dataType: 'JSON',
                data: {}
            }).done(function(response) {
